---
title: "Mixture Model Quantile treatment effect with Generalized Pareto Distribution"
author: "Arnab Aich"
date: "`r Sys.Date()`"
output: 
  pdf_document
...
dTotal<- nimbleFunction(
  run = function(x=double(0),weights = double(1),mu=double(1)
                 ,eta=double(1),u=double(0),sigma=double(0)
                 ,xi=double(0),log = integer(0, default = 0)) 
  {
    ...
    {
      P=sum(pnorm(u, mean = mu, sd = eta)*weights)
...
data_sim <- function(r,N,beta,wt1,wt0,theta0,theta1,eta0,gamma0,gamma1,eta1,delta0,delta1,sigma,xi)
{
  ...
  repeat{
    ...
    ### y(0)
       ...
       y02 = u0+((sigma[1]/xi[1])*(runif(N)**(-xi[1])-1)) 
       y0 = ifelse(runif(N)<p0,y01,y02)
    ### y(1)
       y11 = ifelse(runif(N)<wt1,rnorm(N,mu1[,1],eta1[1]),rnorm(N,mu1[,2],eta1[2]))
       y12 = u1+((sigma[2]/xi[2])*(runif(N)**(-xi[2])-1))
       y1 = ifelse(runif(N)<p1,y11,y12)
    ### y
      y =  ifelse(t==0,y0,y1)
      out$Y[,n_rep] = y
  # plots
...
DATA=data_sim(r=5,N=200,beta= c(-0.03,-0.4,0.12,0.55),wt1=0.35,theta0=c(7,10),theta1=c(2.5,2),eta1 = eta1_true,wt0=0.5,gamma0=c(5.5,8),gamma1 = c(2,3),eta0 = eta0_true ,delta0=c(6.5,8),delta1 = c(7.5,9.5), sigma = sigma_true,xi= xi_true)
...
code_trt=nimbleCode(
  ...
  weights[1:m] <- stick_breaking(v[1:(m-1)])
     ...
     {
       eta[j] ~ dbeta(3,2)
       theta0[j] ~ dnorm(mean = 8,sd  = 4)
       theta1[j] ~ dnorm(mean = 2,sd  = 4)
     }
  # co effcient for u
     delta0 ~ dnorm(mean = 5,sd  = 1)
     delta1 ~ dnorm(mean = 5,sd  = 1)
          
  # additional Parameters for Tail Part
            xi ~ T(dnorm(0.3, sd = 0.1),-0.5,Inf)
  ...
  #  GPD Regression
           ...
           y[i] ~ dTotal(weights[1:m],mu[i,1:m],eta[1:m],u[i],sigma,xi)
    }
 })
...
code_con=nimbleCode(
  ...
  weights[1:m] <- stick_breaking(v[1:(m-1)])
     ...
     {
       eta[j] ~ dbeta(5,1)
       theta0[j] ~ dnorm(mean = 6,sd  = 5)
       theta1[j] ~ dnorm(mean = 2,sd  = 5)
     }
  # co effcient for u
     delta0 ~ dnorm(mean = 6,sd  = 1)
     delta1 ~ dnorm(mean = 6,sd  = 1)
          
  # additional Parameters for Tail Part
            xi ~ T(dnorm(0.4, sd = 0.1),-0.5,Inf)
          sigma ~ dgamma(shape = 2.5,scale = 2)
  #  Distribution for data
    for(i in 1:n)
    {
       # mixture model Regression
...
Sim <- function(data,code,epsilon,n_mix,true,p_qte,ISE,mcmc.plot = FALSE,n_iter,n_burnin,n_chains,n_thin)
{
  # Model Constants
  ...
  inits = list(
             ...
             ,u=rep(quantile(data$y,0.90,na.rm =TRUE),consts$n)
             ,sigma=2,xi=0.2
             ,delta0 = 7,delta1=9
             ,v  = rbeta(consts$m, 1, 1))
  # running mcmc
  nimble_out <- nimbleMCMC(code, constants = consts, data = data
                          ,monitors = c("alpha","weights","eta","theta1","theta0","delta0","delta1","sigma","xi")
...
id = min(which(cum_sum(mix_par$weights)>1-epsilon),which(mix_par$weights<epsilon))
...
# estimated parameters for Tail
delta0_hat = estimates["delta0"]
delta1_hat = estimates["delta1"]
u_hat = mean(delta0_hat+data$X*delta1_hat)
sigma_hat = estimates["sigma"]
xi_hat = estimates["xi"]
my.list$GPD_parameters = data.frame( u = u_hat, sigma = sigma_hat , xi = xi_hat,row.names = NULL )
# MCMC plot
if(mcmc.plot == TRUE)
  {S = ggs(nimble_out$samples[,c('alpha','sigma','xi',"delta0","delta1")])
...
xi = samples[,"xi"]
 ...
 ISE = sapply(seq(1,dim(samples)[1]),function(j){mean(sapply(grid,function(x,j){(dTotal(x,weights[j,],mu[j,],eta[j,],u[j],sigma[j],xi[j])-dTrue(x,true$weights,true$mu,true$eta,true$u,true$sigma,true$xi))**2},j))})
  my.list$Integrated_Squared_error = mean(ISE)
  # density comparison
  grid = seq(min(data$y),max(data$y), length.out = length(data$y))
  f_hat<- sapply(grid,dTotal,weights_hat,mu_hat,eta_hat,u_hat,sigma_hat,xi_hat)
  ...
  set =  sapply(rep(1,10000),rTotal,weights_hat,mu_hat,eta_hat,u_hat,sigma_hat,xi_hat)
...
```{r}
Analyze=function(out,n_mix=5,true,p=0.95,mcmc.plot = FALSE,post_density = FALSE,n_iter=1000,n_burnin=0,n_chains=1,n_thin=1)
{
  ...
  for(i in 1:n_rep)
  {
    ...
    u_m[i] =  out[[i]]$GPD_parameters$u
    s_m[i] = out[[i]]$GPD_parameters$sigma
    x_m[i] = out[[i]]$GPD_parameters$xi
    e_q[i,] = out[[i]]$estimated_quantile
    ise = ise + out[[i]]$Integrated_Squared_error
    if(mcmc.plot == TRUE)
    {
      diagnostics$trace_plots[[i]] = out[[i]]$trace_plot
      diagnostics$density_plots[[i]] = out[[i]]$density_plot
    }
     used = used+1
    }
  }
  ...
  MC_SE_alpha = empirical_SE_alpha/sqrt(n_total)
  ...
  final$mcmc.diagnostics = diagnostics
   final$posterior_estimate = list(alpha_hat,weights_hat,mu_hat,eta_hat,u_hat,sigma_hat,xi_hat)
   names(final$posterior_estimate ) = c("alpha","weights","mu","eta","u","sigma","xi")
  # density Plot of parameters
   if(post_density ==TRUE)
  {
     samples = data.frame('alpha'=a_m,'u'=u_m,'sigma'= s_m,'Xi'=x_m)
  posterior = list()
  posterior$alpha = ggplot(samples,aes(x=alpha),..density..)+
                    geom_histogram(aes(y=..density..),color="blue",fill="lightgreen")+
                      geom_density(color="red",size=1)+xlab(expression(alpha))+ylab('Density')+
                                                                                    theme_classic()
                      
  posterior$u = ggplot(samples,aes(x=u))+
                   geom_histogram(aes(y=..density..),color="blue",fill="lightgreen")+
                      geom_density(color="red",size=1)+xlab(expression(u))+
                                                                                    theme_classic()
  posterior$sigma = ggplot(samples,aes(x=sigma),..density..)+
                      geom_histogram(aes(y=..density..),color="blue",fill="lightgreen")+
                      geom_density(color="red",size=1)+xlab(expression(sigma))+ylab('Density')+
                                                                                    theme_classic()
                      
  posterior$xi =  ggplot(samples,aes(x=Xi),..density..)+
                 geom_histogram(aes(y=..density..),color="blue",fill="lightgreen")+
                      geom_density(color="red",size=1)+xlab(expression(xi))+ylab('Density')+
                                                                                    theme_classic()
  final$Posterior_densities = posterior
   }
   colnames(e_q) = NULL
  final$quantiles = e_q
 # density comparison
  r_true = rTrue(n_total,true$weights,true$mu,true$eta,true$u,true$sigma,true$xi)
  grid = seq(min(r_true),max(r_true+5), length.out = n_total)
  f_hat <- sapply(grid,dTotal,weights_hat,mu_hat,eta_hat,u_hat,sigma_hat,xi_hat)
  f_true = sapply(grid, dTrue,true$weights,true$mu,true$eta,true$u,true$sigma,true$xi)
 
 final$Density_Plots = ggplot()+xlab('')+theme_classic()+xlim(min(r_true),max(true$u,u_hat)+5)+
  ...
  # # plot zoom
   final$zoomed_Density_Plots = ggplot()+xlab('')+theme_classic()+xlim(true$u-2,true$u+2)+
    geom_histogram(aes(r_true,..density..),color="black",fill="beige",bins = 30)+
    geom_line(aes(x=grid,y=f_true ,color="TRUE"),linewidth=0.7)+
    geom_line(aes(x=grid,y=f_hat,color = "Fitted"),linewidth=0.75,linetype = 'dashed')+
    geom_vline(aes(xintercept = true$u,color='TRUE'),linewidth=0.7)+
...
```{r}
Report = function(n_rep,N,n_mix,beta,wt1,theta0,theta1,eta1_true,wt0,gamma0,gamma1,eta0_true ,delta0,delta1,sigma_true,xi_true,p_QTE,ISE,epsilon,mcmc.plot = FALSE,post_density = FALSE,n_iter,n_burnin,n_chains,n_thin,n_cores,seed)
{
   my.list = list()
  t1=Sys.time()
  set.seed(seed)
...
# setting up parallel computing
  ...
  out_treat <- parLapply(my.cluster,treatment_data,Sim,code_trt,epsilon,n_mix,true_trt,p_QTE,ISE,mcmc.plot,n_iter,n_burnin,n_chains,n_thin)
  out_con <- parLapply(my.cluster,control_data,Sim,code_con,epsilon,n_mix,true_con,p_QTE,ISE,mcmc.plot,n_iter,n_burnin,n_chains,n_thin)
   stopCluster(my.cluster)
  
  trt_output = Analyze(out_treat,n_mix,true_trt,p_QTE,mcmc.plot,post_density,n_iter,n_burnin,n_chains,n_thin)
  con_output = Analyze(out_con,n_mix,true_con,p_QTE,mcmc.plot,post_density,n_iter,n_burnin,n_chains,n_thin)
  ...
  Treatment$mcmc.diagnostics = trt_output$mcmc.diagnostics
  Treatment$Posterior_density = trt_output$Posterior_densities
   ## density comparison
  ...
  Control$mcmc.diagnostics = con_output$mcmc.diagnostics
  Control$Posterior_density = con_output$Posterior_densities
   ## density comparison
...
Final = Report(n_rep,N,n_mix,beta,wt1,theta0,theta1,eta1_true,wt0,gamma0,gamma1,eta0_true ,delta0,delta1,sigma_true,xi_true,p_QTE,ISE,epsilon,mcmc.plot = FALSE,post_density = FALSE,n_iter,n_burnin,n_chains,n_thin,n_cores,seed)
```

```{r Data Description, eval=FALSE, include=FALSE}
