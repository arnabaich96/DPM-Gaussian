...
code_trt=nimbleCode(
  {
       eta~ dbeta(3,2)
       theta0 ~ dnorm(mean = 8,sd  = 2)
       theta1 ~ dnorm(mean = 2,sd  = 2)
  #  Distribution for data
    for(i in 1:n)
    {
        y[i] ~ dnorm(theta0+theta1*X[i],eta)
    }
 })
...
code_con=nimbleCode(
  {
       eta ~ dbeta(4,1)
       theta0 ~ dnorm(mean = 6.5,sd  = 2)
       theta1 ~ dnorm(mean = 2.5,sd  = 2)
  #  Distribution for data
    for(i in 1:n)
    {
         y[i] ~ dnorm(theta0+theta1*X[i],sd = eta)
    }
 })
```
...
my.list$Parameters = data.frame(theta1=theta1_hat,theta0=theta0_hat,mu=mu_hat,eta = eta_hat)
# Integrated standard error
...
eta = samples[,"eta"]
 ...
 ISE = sapply(seq(1,dim(samples)[1]),function(j){mean(sapply(grid,function(x,j){(dnorm(x,mu[j],eta[j])-dTrue(x,true$weights,true$mu,true$eta,true$u,true$sigma,true$xi))**2},j))})
  my.list$Integrated_Squared_error = mean(ISE)
  # density comparison
  grid1 = seq(min(data$y),max(data$y), length.out = length(data$y))
  f_hat1<- sapply(grid1,dnorm,mu_hat,eta_hat)
  ...
  set =  sapply(rep(1,10000),rnorm,mu_hat,eta_hat)
...
{ # estimated parameter value
  ...
  for(i in 1:n_rep)
  {
      e_m[i] = out[[i]]$Parameters$eta
      m_m[i] = out[[i]]$Parameters$mu
  ...
  final$posterior_estimate = c(mu_hat,eta_hat)
   names(final$posterior_estimate ) = c("mu","eta")
  # Quantiles                                                                                
   colnames(e_q) = NULL
  final$quantiles = e_q
 # density comparison
  ...
  f_hat <- sapply(grid,dnorm,mu_hat,eta_hat)
...
# setting up parallel computing
  ...
  clusterExport(my.cluster,c("dgpd","rgpd","dTrue","rTrue","cum_sum","code_con","code_trt","Sim"
  ),envir = .GlobalEnv)
